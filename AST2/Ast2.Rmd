...
$\hat{\boldsymbol{\beta}}_{\text{ols}}$ is:

\begin{table}[ht]
\centering
\begin{tabular}{rrrrrrrrrrrrrrrr}
  \hline
 & M & So & Ed & Po1 & Po2 & LF & M.F & Pop & NW & U1 & U2 & GDP & In
eq & Prob & Time \\ 
  \hline
1 & 0.19 & 0.21 & 0.64 & 0.31 & 0.45 & -0.09 & 0.04 & 0.08 & -0.01 & 
-0.03 & 0.15 & 0.10 & 0.77 & -0.26 & 0.06 \\ 
   \hline
\end{tabular}
\end{table}
...
$\hat{\boldsymbol{\beta}}_{\text{Bayes}}$ is:
...
eq & Prob & Time \\ 
  \hline
1 & 0.18 & 0.22 & 0.61 & 0.23 & 0.48 & -0.09 & 0.04 & 0.09 & -0.02 & 
-0.03 & 0.14 & 0.11 & 0.74 & -0.26 & 0.06 \\ 
   \hline
\end{tabular}
\end{table}
```{r}
...
We iterativly go through step 1 to step 11 below to sample $\beta_0,\ldots,\beta_5, \gamma_1, \ldots, \gamma_5$ using Metropolis-Hastings algorithms:

1. Update $\beta_0$:
   a. Sample $\beta_0^* \sim N(\beta_0^{(s)}, 16)$
   b. compute the log acceptance ratio\footnote{In many cases, computing the ratio $r$ directly can be numerically unstable, a problem that often can be remedied by computing the logarithm of $r$}\footnote{We are using symmeric proposal distribution, so the proposal distribution canceled out}
   \[
   log(r)=log\frac{p(\mathbf{Y}|\mathbf{X}, \beta_1^{(s)},\ldots,\beta_5^{(s)},\gamma_1^{(s)},\ldots,\gamma_5^{(s)}, \beta_0^*)}{p(\mathbf{Y}|\mathbf{X}, \beta_1^{(s)},\ldots,\beta_5^{(s)},\gamma_1^{(s)},\ldots,\gamma_5^{(s)}, \beta_0^{(s)})}
   .\] 
   c. set $\beta_0^{(s+1)}$ to $\beta_0^{*}$ or $\beta_0^{(s)}$ with probability min$(1,r)$ and max$(0,1-r)$.
2. Update $\beta_1$:
   a. Sample $\beta_1^* \sim N(\beta_1^{(s)}, 4)$
   b. compute the log acceptance ratio
   \[
   log(r)=log\frac{p(\mathbf{Y}|\mathbf{X}, \beta_0^{(s+1)}, \beta_2^{(s)},\ldots,\beta_5^{(s)},\gamma_1^{(s)},\ldots,\gamma_5^{(s)}, \beta_1^*)}{p(\mathbf{Y}|\mathbf{X}, \beta_0^{(s+1)}, \beta_2^{(s)},\ldots,\beta_5^{(s)},\gamma_1^{(s)},\ldots,\gamma_5^{(s)}, \beta_1^{(s)})}
   .\] 
   c. set $\beta_1^{(s+1)}$ to $\beta_1^{*}$ or $\beta_1^{(s)}$ with probability min$(1,r)$ and max$(0,1-r)$.
...
11. Update $\gamma_5$:
   ...
   b. compute the log acceptance ratio
   \[
   log(r)=log\frac{p(\mathbf{Y}|\mathbf{X}, \beta_0^{(s+1)}, \ldots,\beta_5^{(s+1)},\gamma_1^{(s+1)},\ldots,\gamma_4^{(s+1)}, \gamma_5^*)}{p(\mathbf{Y}|\mathbf{X}, \beta_0^{(s+1)}, \ldots,\beta_5^{(s+1)},\gamma_1^{(s+1)},\ldots,\gamma_4^{(s+1)}, \gamma_5^{(s)})}
   .\] 
   c. set $\gamma_5^{(s+1)}$ to $\gamma_5^{*}$ or $\gamma_5^{(s)}$ with probability min$(1,r)$ and max$(0,1-r)$.

<!--
I'm going to abuse the notation and by writing $\boldsymbol{\beta} = (\beta_1,\beta_2,\beta_3,\beta_4, \beta_5)^T$ since $beta_0$ has aand let the posterior distribution of $\beta_0, \boldsymbol{\beta}$ and $\boldsymbol{\gamma}$ be $p(\beta_0|\mathbf{X,Y},\boldsymbol{\beta},\gamma)$, $p(\boldsymbol{\beta}|\mathbf{X,Y},\beta_0,\gamma)$ and $p(\boldsymbol{\gamma}|\mathbf{X,Y}, \beta_0, \boldsymbol{\beta})$. We use Metropolis-Hastings algorithm to iterate through the following 3 steps and approach those three posterior distributions:
...
The sampling results are not very satisfying, only $\beta_0$ seems fine to me. $\beta_1, \beta_2$ shows too high of accptance rate and also too high of autocorrelation. But given that $\beta_1\gamma_1$ and $\beta_2\gamma_2$ are stationarity at 0, we can accept that. However, for  $\beta_4\gamma_4$, it's not acheiving stationarity.
